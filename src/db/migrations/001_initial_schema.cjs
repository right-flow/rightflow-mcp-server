 ...
 *
 * RightFlow-ready: Includes tenant_type for future integration
 */
...
exports.up = async function(knex) {
  ...
  await knex.schema.createTable('users', (table) => {
    ...
    table.string('grow_subscription_id', 255).nullable();
  ...
  await knex.schema.createTable('forms', (table) => {
    ...
    table.jsonb('stations').defaultTo('[]').notNullable();  // RightFlow station support
    table.jsonb('settings').defaultTo('{}').notNullable();
