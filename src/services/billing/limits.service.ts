...
export interface Plan {
  ...
  max_forms: number; // -1 = unlimited
  max_responses_monthly: number;
...
export class LimitsService {
  ...
  async checkLimit(params: {
    userId: string;
    action: 'create_form' | 'submit_response' | 'upload_file';
    resourceSize?: number; // for storage check in bytes
   ...
   * Check forms limit
   */
  private checkFormsLimit(plan: Plan, usage: UsageMetrics): CheckLimitResult {
    // -1 means unlimited
    if (plan.max_forms === -1) {
      return { allowed: true };
    }
   ...
   */
  private checkResponsesLimit(plan: Plan, usage: UsageMetrics): CheckLimitResult {
    // -1 means unlimited
   ...
   */
  async getUserPlanLimits(userId: string): Promise<UserPlanLimits> {
    const user = await this.db('users')
      .where({ id: userId })
      .first();
