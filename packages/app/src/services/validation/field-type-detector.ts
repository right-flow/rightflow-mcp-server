...
import type { FieldType, ValidatorConfig } from '@/types/fields';
import {
  type DetectionMatch,
 ...
 */
function getHtmlTypesForFieldType(fieldType: FieldType): string[] {
  switch (fieldType) {
    case 'text':
 ...
 */
function calculateMatchScore(label: string, pattern: string): number {
  ...
  for (const [fieldTypeId, definition] of Object.entries(
    ...
    if (bestPatternScore >= SCORE_THRESHOLD) {
      matches.push({
        fieldTypeId,
        displayName:
          FIELD_TYPE_DISPLAY_NAMES[fieldTypeId] || fieldTypeId,
