...
import { isBillingEndpoint, getMockResponse } from './mockData';

const API_BASE_URL = import.meta.env.VITE_API_BASE_URL || 'http://localhost:3000';
const USE_MOCK_DATA = import.meta.env.VITE_USE_MOCK_BILLING !== 'false'; // Default: true
...
class ApiClient {
  ...
  /**
   * Handle API errors with retry logic and mock data fallback
   */
  private async handleError(error: AxiosError): Promise<any> {
    const config = error.config as AxiosRequestConfig & { _retryCount?: number };

    // Handle 404 errors on billing endpoints with mock data
    if (
      USE_MOCK_DATA &&
      error.response?.status === 404 &&
      config?.url &&
      isBillingEndpoint(config.url)
    ) {
      console.log(`🔧 [Mock Data] Returning mock data for: ${config.url}`);
      const mockData = getMockResponse(config.url, config.method?.toUpperCase() || 'GET');
