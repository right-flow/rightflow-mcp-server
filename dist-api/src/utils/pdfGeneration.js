 ...
 */
import { PDFDocument, rgb, PDFName, PDFNumber, PDFBool, PDFString } from 'pdf-lib';
 ...
 */
function wrapWithRTLMarkers(text) {
    if (!text)
        return text;
 ...
 */
function addCustomFieldMetadata(fieldDict, field) {
    try {
 ...
 */
function createTextField(pdfDoc, page, field, hebrewFont) {
 ...
 */
function createCheckboxField(pdfDoc, page, field, checkboxStyle = 'check') {
    const form = pdfDoc.getForm();
    // Create checkbox
 ...
 */
function createRadioGroupFromFields(pdfDoc, pages, groupFields) {
 ...
 * @param hebrewFont - Embedded Hebrew font
 */
function createDropdownField(pdfDoc, page, field, hebrewFont) {
    const form = pdfDoc.getForm();
    // Create dropdown
 ...
 */
async function createSignatureField(pdfDoc, page, field) {
 ...
 */
export async function generateFillablePDF(originalPdfFile, fields, options) {
    try {
        ...
        const uniqueFieldNames = new Map(); // Maps field.id -> unique PDF field name
        ...
        const getUniqueFieldName = (field) => {
        ...
        const createdRadioGroups = new Set();
        ...
        for (const [pageNum, pageFields] of Object.entries(fieldsByPage)) {
            ...
            for (const field of pageFields) {
                ...
                if (field.type === 'text') {
                    ...
                    const uniqueGroupName = getUniqueFieldName({ ...field, name: groupName });
 ...
 */
export async function downloadPDF(pdfBytes, filename, fields) {
 ...
 */
export function ensureFieldNames(fields) {
    const typeCounters = {};
 ...
 */
export function validateFieldsForPDF(fields) {
    const errors = [];
    if (fields.length === 0) {
