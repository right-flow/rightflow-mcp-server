 ...
 */
import { mapFieldsToHtml, createFieldGroups, detectFormDirection, groupFieldsIntoRows, } from './field-mapper';
import { generateDocsFlowCSS, generateFormJS } from './html-generator-css';
import { sanitizeHexColor, sanitizeFontSize, sanitizeFontWeight, sanitizeFontStyle, sanitizeTextAlign, } from '@/utils/inputSanitization';
/**
 ...
 */
function generateWelcomePageHtml(rtl, welcomeConfig) {
    const config = {
        enabled: true,
 ...
 */
function generateFieldHtml(field, includeValidation) {
 ...
 */
function generateValidationAttrs(field) {
  ...
  `.trim();
    switch (field.type) {
        ...
        case 'static-text': {
            ...
            const borderStyle = field.borderWidth !== undefined &&
 ...
 */
function generateTabsHtml(totalPages, includeWelcome) {
 ...
 */
function generateNavigationHtml(totalPages, rtl, includeWelcome) {
    const totalTabs = includeWelcome ? totalPages + 1 : totalPages;
    if (totalTabs <= 1) {
 ...
 */
function generatePageHtml(pageNum, fields, groups, includeValidation, rtl, isActive = false) {
    // Filter groups that have fields on this page
    const pageFieldIds = new Set(fields.map((f) => f.id));
    const pageGroups = groups.filter((g) => g.fields.some((fid) => pageFieldIds.has(fid)));
    let html = `
 ...
 */
export async function generateHtmlFormTemplate(fields, options = {}) {
...
`;
    ...
    for (let i = 0; i < pageNumbers.length; i++) {
        ...
        htmlContent += generatePageHtml(i + 1, // Use 1-based index for display
        pageFields, groups, finalOptions.includeValidation, finalOptions.rtl, !includeWelcome && i === 0);
    }
    // If no pages (shouldn't happen), create single page with all fields
    if (pageNumbers.length === 0) {
        htmlContent += generatePageHtml(1, htmlFields, groups, finalOptions.includeValidation, finalOptions.rtl, !includeWelcome);
    }
    htmlContent += `
    </div>
