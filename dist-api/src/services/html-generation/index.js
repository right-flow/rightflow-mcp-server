 ...
 */
export async function generateHtmlForm(fields, options = {}, onProgress) {
    ...
    if (method === 'ai' || method === 'auto') {
        try {
            ...
            console.warn('[HTML Generation] AI generation failed, falling back to template:', error);
 ...
 */
export function downloadHtmlFile(result, filename) {
 ...
 */
export function previewHtmlInNewTab(result) {
    const blob = new Blob([result.htmlContent], { type: 'text/html;charset=utf-8' });
    const url = URL.createObjectURL(blob);
 ...
 */
export function createHtmlDataUrl(result) {
 ...
 */
export async function downloadFormPackageZip(pdfFile, fields, htmlResult, filename, formMetadata) {
